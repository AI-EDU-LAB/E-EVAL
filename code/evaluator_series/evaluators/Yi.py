...
class Yi_Evaluator(Evaluator):
    def __init__(self, choices, k, model_name, device):
        super(Yi_Evaluator, self).__init__(choices, model_name, k)
    ...
    def eval_subject(self, subject_name, test_df, dev_df=None, few_shot=False, save_result_dir=None,cot=False):
        ...
        for row_index, row in tqdm(test_df.iterrows(),total=len(test_df)):
            ...
            input_ids = self.tokenizer.apply_chat_template(conversation=full_prompt, tokenize=True, add_generation_prompt=True, return_tensors='pt')
            output_ids = self.model.generate(input_ids.to('cuda'))
            response_str = self.tokenizer.decode(output_ids[0][input_ids.shape[1]:], skip_special_tokens=True)
            if cot:
